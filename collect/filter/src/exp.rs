...
fn run_rayon(input: &[u64], heavy: bool) -> Output {
    use rayon::prelude::*;
    match heavy {
        true => Output::Vec(input.into_par_iter().copied().filter(h_f).collect()),
        false => Output::Vec(input.into_par_iter().copied().filter(l_f).collect()),
    }
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder) -> Output {
    ...
    match heavy {
        true => Output::Vec(par.filter(h_f).collect()),
        false => Output::Vec(par.filter(l_f).collect()),
    }
}
