...
fn run_rayon(input: &[u64], heavy: bool) -> Output {
    use rayon::prelude::*;
    match heavy {
        true => Output::Vec(input.into_par_iter().filter(f).map(h_m).collect()),
        false => Output::Vec(input.into_par_iter().filter(f).map(l_m).collect()),
    }
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder) -> Output {
    ...
    match heavy {
        true => Output::Vec(par.filter(f).map(h_m).collect()),
        false => Output::Vec(par.filter(f).map(l_m).collect()),
    }
}
