...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder, list: bool) -> Output {
    ...
    match (heavy, list) {
        (true, false) => Output::Vec(par.filter(f).map(h_m).flat_map(fl).filter_map(fm).collect()),
        ...
        (false, false) => Output::Vec(par.filter(f).map(l_m).flat_map(fl).filter_map(fm).collect()),
