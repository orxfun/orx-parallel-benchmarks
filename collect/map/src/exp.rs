...
fn run_seq(input: &[u64], heavy: bool) -> Output {
    match heavy {
        true => Output::Vec(input.iter().map(h_m).collect()),
        false => Output::Vec(input.iter().map(l_m).collect()),
    }
}
...
fn run_rayon(input: &[u64], heavy: bool, list: bool) -> Output {
    use rayon::prelude::*;
    match (heavy, list) {
        (true, false) => Output::Vec(input.into_par_iter().map(h_m).collect()),
        (true, true) => Output::VecList(input.into_par_iter().map(h_m).collect_vec_list()),
        (false, false) => Output::Vec(input.into_par_iter().map(l_m).collect()),
        (false, true) => Output::VecList(input.into_par_iter().map(l_m).collect_vec_list()),
    }
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder, list: bool) -> Output {
    ...
    match (heavy, list) {
        (true, false) => Output::Vec(par.map(h_m).collect()),
        (true, true) => Output::VecVec(par.map(h_m).collect::<Vec2<_>>().into()),
        (false, false) => Output::Vec(par.map(l_m).collect()),
        (false, true) => Output::VecVec(par.map(l_m).collect::<Vec2<_>>().into()),
    }
}
